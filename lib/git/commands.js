...
exports.extractTags = function (tag_name_regex, repo_url, cwd, cb) {
  run_command('git ls-remote --tags' + ' ' + repo_url, cwd, function (err, output) {
    var extracted_tags = [];
    var one_tag;
    var full_regex_tag = new RegExp("refs\/tags\/(" + tag_name_regex + ")\\^\\{\\}", "ig")
    while ((one_tag = full_regex_tag.exec(output)) !== null) {
...
exports.listAdditionalPropertyFiles = function(modifiedRecords, cwd, cb) {
  ...
  function is_modified_properties(modifiedRecords, file) {
  ...
  run_command('git ls-tree --name-status -r HEAD', cwd, function(err, output) {
    ...
    var files = output.split('\n');
    files.forEach(function(file) {
      if(!is_modified_properties(modifiedRecords, file)) {
        records.push({'type': 'M', 'path': file});
      }
    });
